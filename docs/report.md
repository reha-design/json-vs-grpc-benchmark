# 다중 프레임워크 HTTP/2 & gRPC 성능 비교 보고서 (Java 25 및 Node.js 추가)

## 1. 개요
* **목적**: Bun(Elysia.js), Go(Native/Gin), Python(FastAPI), Node.js(Express/Fastify), Kotlin(Spring Boot - Java 25 가상 스레드 활성화) 환경에서 HTTP/2 (TLS) 기반 JSON API(GET/POST/QUERY)와 gRPC(Protobuf)의 성능 격차 정량적 분석.
* **통제 변수**: 
...
* **성능 평가**: 
  - 단일 조회 GET에서 **12,070 RPS**로 최상위권 기록.
...
* **성능 평가**:
  ...
  - gRPC의 성능이 **3,826 RPS** (단일), **2,398 RPS** (대량)로 타 플랫폼 대비 압도적으로 우수.
* **장단점**: 기계어 빌드 특성상 JSON, gRPC 모두 최상위권의 안정적인 성능 유지.

### 2.3 Node.js (Express & Fastify)
* **런타임**: Node 20+ (V8 Engine)
* **특징**: Single-threaded 이벤트 루프 기반 비동기 I/O.
* **성능 평가**:
  - **Fastify (7,364 RPS)** 가 **Express (6,207 RPS)** 대비 약 **18% 높은 RPS** 성능을 보임. Fastify의 내부 스키마 기반 직렬화(fast-json-stringify) 엔진이 Jackson 및 V8 내장 stringify 대비 높은 최적화를 이룸.
  - Express는 전통적인 HTTP/1.1 호환 계층의 오버헤드로 인해 동급 비동기 대비 다소 지연이 더 발생함.
* **장단점**: 대중성은 Express가 높으나 성능 관점에선 Fastify 도입이 유리.

### 2.4 Python (FastAPI)
* **런타임**: CPython 3.14 + uvicorn[standard]
* **특징**: ASGI 규격 기반 비동기(AsyncIO) 처리.
* **성능 평가**:
  - 프레임워크 추상화 레이어로 인해 단일 조회 GET 성능 하락 (`0.56ms`).
  - 대량 데이터(27KB) 조회 시 JSON GET(`2.49ms`) 대비 gRPC(`0.90ms`)가 **2.7배 이상 빠름**.
* **장단점**: 대량의 I/O 및 데이터 전송 구조에서 gRPC 도입 효과가 4대 플랫폼 중 가장 높음.

### 2.5 Kotlin (Spring Boot - Java 25)
* **런타임**: Eclipse Temurin OpenJDK 25 (JVM) + Embedded Tomcat
* **특징**: IOC/DI 컨테이너 기반 및 **`spring.threads.virtual.enabled: true`** 가상 스레드 최적화 적용.
* **성능 평가**:
  - Java 17 대비 단일 gRPC 조회 성능이 **1.25ms -> 0.28ms(3,610 RPS)** 로 **4배 이상 속도 수직 상승**.
  - **검색 (QUERY) 시나리오**에서는 Java 21(`0.56ms`) 대비 더욱 향상된 **`0.39ms` (2,536 RPS)** 기록. JVM 25 엔진의 내부 락/모니터 핀홀 병목 최적화 효과 확인.
